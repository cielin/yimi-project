...
{{ Form::open(array('route' => array('portfolios.update', $portfolio), 'files' => true, 'role' => 'form')) }}
			...
			<div class="col-md-8">
				...
				<div class="post-title">
					<div class="form-group">
						<input name="title" type="text" class="form-control" placeholder="作品标题" value="{{ $portfolio->title }}">
					</div>
					<div class="form-group input-group mb-3">
						...
						<select class="custom-select" id="did" name="did">
							...
							<option value="{{ $designer->id }} @if ($portfolio->designer_id == $designer->id) selected @endif">{{ $designer->name }}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="post-editor form-group">
					<textarea id="editor" name="description" class="form-control" rows="12">{{ $portfolio->description }}</textarea>
				</div>
							...
							<div class="row">
								<div class="col-md-3">
									<div class="thumbnail">
										...
										@else
										{{ Html::image('storage' . config('imgattrs.portfolio_images.thumbnail') . '/thumb_' . $portfolio->image, $portfolio->title) }}
										@endif

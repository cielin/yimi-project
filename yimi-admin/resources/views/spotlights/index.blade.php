...
<div class="row main-content">
	<div class="col-md-12">
						...
						<td width="120">
							<a href="">
								<span class="img-avatar">
									<img src="{{ asset('storage/thumbs/spotlights/thumb_' . $spotlight->image) }}" alt="{{ $spotlight->title }}">
								</span>
							</a>
						</td>
						<td>{{ $spotlight->title }}</td>
						<td>{{ $spotlight->created_at }}</td>
						<td>{{ $spotlight->updated_at }}</td>
						<td>
							<a class="btn btn-primary btn-sm" href="{{ URL::route('spotlights.edit', $spotlight->id) }}">编辑</a>
							<a class="btn btn-danger btn-sm remove-record" href="javascript:void(0);" data-toggle="modal" data-url="{{ route('spotlights.destroy', $spotlight) }}" data-id="{{ $spotlight->id }}" data-target="#custom-width-modal">删除</a>
						</td>
					</tr>
					@endforeach
				</tbody>
		...
		<nav class="footer-nav">
			<?php echo $spotlights->appends(request()->except('page'))->links(); ?>
		</nav>
	</div>
</div>
<!-- End Main Content -->
